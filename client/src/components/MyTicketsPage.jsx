...
export default function MyTicketsPage() {
  ...
  return (
    <main id="main" className="section" style={{ minHeight: '60vh' }}>
      ...
      {loading ? (
        <p style={{ textAlign: 'center', color: 'var(--text-muted)' }}>Loading tickets...</p>
      ) : tickets.length > 0 ? (
        <div className="tickets-grid" role="list">
          {tickets.map((order) => (
            order.tiers.map((tier, index) => (
              <div key={`${order._id}-${index}`} role="listitem">
                {/* Reusing existing TicketCard but adapting data to fit its expected props structure */}
                <TicketCard ticket={{
                  id: `${order._id}-${index}`,
        ...
        <p style={{ textAlign: 'center', color: 'var(--text-muted)' }}>You haven't purchased any tickets yet.</p>
      )}
    </main>
  );
}
